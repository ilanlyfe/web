 ...
 */
interface LinkProps {
  variant?: "primary" | "secondary" | "menu" | "action" | "mobile" | string;
  className?: string;
  active?: boolean;
  href?: string;
  text?: string;
...
const Link: FC<LinkProps> = ({ variant, className, href = "", text, icon = "", iconPlacement, color, onClick, active }) => {
  switch (variant) {
    case "menu":
      return (
        <A href={href}>
          <a onClick={onClick} className={`${className} flex items-center xmd:mr-6 font-medium leading-6 text-gray-600  `}>
            <div
              className={`${
                active === true ? "bg-blue-300" : ""
              } px-4 py-2  min-h-[44px] flex justify-between flex-shrink flex-grow flex-row items-center cursor-pointer text-decoration transition duration-300 hover:bg-blue-200 rounded-lg`}
            >
              ...
              <div className="min-h-0 flex justify-between min-w-0 flex-shrink z-0 flex-row flex-grow items-center">
                <Type variant="base" color={active ? "#fff" : color}>
                  {text}
                </Type>
              </div>
    ...
    case "action":
      return (
        <button onClick={onClick} className={clsx(className, "flex items-center mr-6 font-medium leading-6 text-gray-600 ")}>
