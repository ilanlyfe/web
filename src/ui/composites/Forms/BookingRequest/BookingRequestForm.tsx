...
import { FormikInput } from "../../../elements/Input/FormikInput";
...
const BookingRequestForm: FC<BookingReqProps> = ({ resourceId, experience }) => {
  ...
  async function handleBookingRequest(request?: any) {
    ...
    if (currentUser != null) {
      ...
      try {
        ...
        await handleBookingRequestCall(bookingRequest);
        return;
      } catch (error) {
        console.log(error);
        return;
  ...
  async function handleBookingRequestCall(bookingRequest: IBookingRequest) {
